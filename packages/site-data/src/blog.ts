export type BlogPost = {
  id: number;
  title: string;
  description: string;
  date: string;
  publishedAt: string;
  readingTime: string;
  slug: string;
  themes: string[];
  imageFormat?: 'jpg' | 'png' | 'webp';
  contentClassName?: string;
};

export const blogPosts: BlogPost[] = [
  {
    id: 23,
    title:
      'Interview with Misaki Takamatsu of Skip and Loafer: The story began with Ryotaro Shiba, and reaches toward the human relationships that emerge after the shine wears away',
    description:
      'A translation of an interview with Misaki Takamatsu about the roots of Skip and Loafer, Shima and Mitsumi, and relationships that begin after the polished surface comes off.',
    date: 'June 21st, 2026',
    publishedAt: '2026-06-21T00:00:00.000Z',
    readingTime: '15 min read',
    slug: 'takamatsu-interview-translation-2',
    themes: ['Skip and Loafer', 'Translation']
  },
  {
    id: 22,
    title: 'The day we walked',
    description:
      'Some days arrive quietly and leave without a trace. Others linger long after they have passed, carried by the wind, hidden in distant memories, or reflected in the changing colors of the sky.',
    date: 'June 2nd, 2026',
    publishedAt: '2026-06-02T00:00:00.000Z',
    readingTime: '5 min read',
    slug: 'the-day-we-walked',
    themes: ['Kemutai Hanashi', 'Fanfiction'],
    imageFormat: 'jpg',
    contentClassName: 'prose-p:italic prose-p:text-slate-700 dark:prose-p:text-slate-300'
  },
  {
    id: 21,
    title: '[Archive] Let\'s talk about Kemutai Hanashi, everything about "normality"',
    description: 'After all, everyone wants to be normal, right?',
    date: 'May 23rd, 2026',
    publishedAt: '2026-05-23T18:03:00.000Z',
    readingTime: '3 min read',
    slug: 'kemutai-hanashi-normality',
    themes: ['Kemutai Hanashi']
  },
  {
    id: 17,
    title: 'My thoughts about Kemutai Hanashi - Chapter 4: Decisions',
    description: 'Sometimes everything comes from a decision.',
    date: 'May 7th, 2026',
    publishedAt: '2026-05-07T00:00:00.000Z',
    readingTime: '10 min read',
    slug: 'kemutai-hanashi-4',
    themes: ['Kemutai Hanashi']
  },
  {
    id: 16,
    title: 'My thoughts about Hoshiai no Sora',
    description: 'To define who we are, our purposes and to go with it.',
    date: 'April 28th, 2026',
    publishedAt: '2026-04-28T00:00:00.000Z',
    readingTime: '5 min read',
    slug: 'hoshiai-no-sora',
    themes: ['Hoshiai no Sora']
  },
  {
    id: 15,
    title: 'I have my words, and time is ticking',
    description:
      'The answer is not found in words but in the endurance of a feeling that has no choice but to exist.',
    date: 'April 26th, 2026',
    publishedAt: '2026-04-26T00:00:00.000Z',
    readingTime: '15 min read',
    slug: 'kemutai-hanashi-fic-1',
    themes: ['Kemutai Hanashi', 'Fanfiction'],
    contentClassName: 'prose-p:italic prose-p:text-slate-700 dark:prose-p:text-slate-300'
  },
  {
    id: 14,
    title: 'My directions',
    description: 'There is something I want to say.',
    date: 'April 10th, 2026',
    publishedAt: '2026-04-10T00:00:00.000Z',
    readingTime: '5 min read',
    slug: 'my-directions',
    themes: ['Personal']
  },
  {
    id: 13,
    title: 'My thoughts about Kemutai Hanashi - Chapter 2 and 3: Relationships and labels',
    description: 'When things cannot be made into words easily.',
    date: 'March 9th, 2026',
    publishedAt: '2026-03-09T00:00:00.000Z',
    readingTime: '10 min read',
    slug: 'kemutai-hanashi-2-3-en',
    themes: ['Kemutai Hanashi'],
    imageFormat: 'jpg'
  },
  {
    id: 17,
    title: 'My thoughts about Kemutai Hanashi - Chapter 1',
    description: 'Where there is smoke, there is fire.',
    date: 'May 14th, 2025',
    publishedAt: '2025-05-14T00:00:00.000Z',
    readingTime: '20 min read',
    slug: 'what-can-i-change',
    themes: ['Personal']
  },
  {
    id: 12,
    title: '「煙たい話：第一章」についての私の考え',
    description: '煙あるところに火あり。',
    date: 'November 15th, 2025',
    publishedAt: '2025-11-15T00:00:00.000Z',
    readingTime: '20 min read',
    slug: 'kemutai-hanashi-1-jp',
    themes: ['Kemutai Hanashi']
  },
  {
    id: 11,
    title: 'My thoughts about Kemutai Hanashi - Chapter 1',
    description: 'Where there is smoke, there is fire.',
    date: 'November 15th, 2025',
    publishedAt: '2025-11-15T00:00:00.000Z',
    readingTime: '20 min read',
    slug: 'kemutai-hanashi-1-en',
    themes: ['Kemutai Hanashi'],
    imageFormat: 'jpg'
  },
  {
    id: 10,
    title: "Takamatsu-sensei's January 13th space - translation",
    description: 'An updated translation of the space that Takamatsu-sensei did on January 13th.',
    date: 'November 13th, 2025',
    publishedAt: '2025-11-13T00:00:00.000Z',
    readingTime: '45 min read',
    slug: 'takamatsu-space-translation',
    themes: ['Skip and Loafer', 'Translation']
  },
  {
    id: 9,
    title: "Yuki Kodama's 25th anniversary as a manga artist: Special project 1 - Interview with Takamatsu Misaki",
    description: 'An interview between authors during the anniversary of Yuki Kodama-sensei.',
    date: 'June 13th, 2025',
    publishedAt: '2025-06-13T00:00:00.000Z',
    readingTime: '20 min read',
    slug: 'flowers-interview',
    themes: ['Skip and Loafer', 'Translation'],
    imageFormat: 'jpg'
  },
  {
    id: 8,
    title: 'Some lines of thoughts I have for Chapter 71',
    description: 'Parents and children, a connection that is complex in its own way.',
    date: 'June 3rd, 2025',
    publishedAt: '2025-06-03T00:00:00.000Z',
    readingTime: '20 min read',
    slug: 'my-thoughts-about-chapter-71',
    themes: ['Skip and Loafer']
  },
  {
    id: 7,
    title: 'Happy birthday Mukai',
    description: 'happy birthday mukai.',
    date: 'May 19th, 2025',
    publishedAt: '2025-05-19T00:00:00.000Z',
    readingTime: '10 min read',
    slug: 'happy-birthday-mukai',
    themes: ['Skip and Loafer', 'Fanfiction'],
    imageFormat: 'jpg'
  },
  {
    id: 18,
    title: 'Autograd Logic',
    description: 'the logic behind how autograd links to optimization through loss minimization',
    date: 'February 24, 2025',
    publishedAt: '2025-02-24T00:00:00.000Z',
    readingTime: '15 min read',
    slug: 'autograd-logic',
    themes: ['Computer Science']
  },
  {
    id: 6,
    title: 'My thoughts about Chapter 68',
    description: 'To grow up is to become a child again.',
    date: 'February 26th, 2025',
    publishedAt: '2025-02-26T00:00:00.000Z',
    readingTime: '20 min read',
    slug: 'my-thoughts-about-chapter-68',
    themes: ['Skip and Loafer']
  },
  {
    id: 5,
    title: 'Thoughts of Shima - the sand and the crab',
    description: 'Just my thoughts about Shima and the way those elements relate to him.',
    date: 'February 26th, 2025',
    publishedAt: '2025-02-26T00:00:00.000Z',
    readingTime: '20 min read',
    slug: 'sand-crab-thoughts',
    themes: ['Skip and Loafer']
  },
  {
    id: 4,
    title: 'Autograph session of Takamatsu-sensei in Taiwan - translation',
    description:
      'About the autograph session which she was invited in two days: February 9th - February 10th.',
    date: 'February 26th, 2025',
    publishedAt: '2025-02-26T00:00:00.000Z',
    readingTime: '20 min read',
    slug: 'takamatsu-autograph-taiwan',
    themes: ['Skip and Loafer', 'Translation']
  },
  {
    id: 3,
    title: 'My thoughts about Chapter 67',
    description: 'Shima, Mitsumi, and the rest of the gang - my personal thoughts about everything.',
    date: 'January 31st, 2025',
    publishedAt: '2025-01-31T00:00:00.000Z',
    readingTime: '30 min read',
    slug: 'my-thoughts-about-chapter-67',
    themes: ['Skip and Loafer']
  },
  {
    id: 19,
    title: 'Misaki Takamatsu speaks about her long-term serialization journey in Skip and Loafer (Translation)',
    description:
      'A translation of the space that Takamatsu-sensei did on an interview, which released on January 2025.',
    date: 'January 31st, 2025',
    publishedAt: '2025-01-31T00:00:00.000Z',
    readingTime: '30 min read',
    slug: 'takamatsu-interview-translation-1',
    themes: ['Skip and Loafer', 'Translation']
  },
  {
    id: 2,
    title: 'Discord archive - About Shima, Mitsumi, Nao; the seashell, sand, and the sea',
    description: 'A thought that I made when a person asked me to analyze them.',
    date: 'January 31st, 2025',
    publishedAt: '2025-01-31T00:00:00.000Z',
    readingTime: '15 min read',
    slug: 'shima-mitsumi-nao-thoughts',
    themes: ['Skip and Loafer']
  },
  {
    id: 20,
    title: 'My 2025 Resolution: Beyond the Roadmaps, Beyond the Timelines',
    description: 'Beyond the Roadmaps, Beyond the Timelines - A reflection on learning efficiency',
    date: 'January 14, 2025',
    publishedAt: '2025-01-14T00:00:00.000Z',
    readingTime: '5 min read',
    slug: 'my-2025-resolution',
    themes: ['Personal']
  },
  {
    id: 1,
    title: '(In-working) Episode 1 commentary - translation',
    description: 'A translation of episode 1 commentary, which released on November 2024.',
    date: 'January 31st, 2025',
    publishedAt: '2025-01-31T00:00:00.000Z',
    readingTime: '10 min read',
    slug: 'episode-1-commentary',
    themes: ['Skip and Loafer', 'Translation']
  }
];

export function getBlogPostBySlug(slug: string) {
  return blogPosts.find((post) => post.slug === slug);
}
