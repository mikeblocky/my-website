...
function getCooldownKey(scope: CooldownScope, request: Request) {
  const fingerprint = [
...
export async function getMessageCooldown(scope: CooldownScope, request: Request): Promise<CooldownState> {
...
export async function assertMessageCooldown(scope: CooldownScope, request: Request) {
...
export async function reserveMessageCooldown(scope: CooldownScope, request: Request) {
  const redis = await getRedisClient()
...
export async function startMessageCooldown(scope: CooldownScope, request: Request): Promise<CooldownState> {
  const redis = await getRedisClient()
  const expiresAtMs = Date.now() + MESSAGE_COOLDOWN_MS
