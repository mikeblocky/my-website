...
import type { SuggestionReference } from '@mikeblocky/site-data';
...
export const GET: RequestHandler = async ({ url }) => {
  ...
  let targetUrl: URL;
  try {
    targetUrl = new URL(resolvedUrlStr);
  } catch (_error) {
  ...
  try {
    ...
    if (!response.ok) {
      return json({ error: `Reference page responded with ${response.status}` }, { status: 502 });
    }

    const contentType = response.headers.get('content-type') || '';
    if (!contentType.includes('text/html')) {
      return json({ error: 'Reference URL is not an HTML page' }, { status: 415 });
    }
    ...
    const image = resolveMaybeRelativeUrl(readMeta(html, 'og:image') || readMeta(html, 'twitter:image'), targetUrl.toString());
    const siteName = readMeta(html, 'og:site_name') || targetUrl.hostname.replace(/^www\./, '');
    const type = readMeta(html, 'og:type');

    const extra = extractExtraDetails(html, targetUrl.toString());
