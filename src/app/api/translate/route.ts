...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!data || typeof data.html !== 'string' || typeof data.targetLanguage !== 'string') {
            return NextResponse.json({ error: 'Missing html or targetLanguage parameter' }, { status: 400 })
        }
...
6. Return only the translated HTML. Ensure it is valid HTML and contains all original tags.`
        ...
        if (!response.ok) {
            ...
            return NextResponse.json({ error: 'Failed to translate content via Gemini API' }, { status: 502 })
        }
        ...
        if (!translatedText) {
            return NextResponse.json({ error: 'No translation returned from Gemini API' }, { status: 502 })
        }
        ...
        return NextResponse.json({ translatedHtml: translatedText })
    } catch (error: any) {
